...
def draw_plot(X, Y, OUT, TITLE, FILLUNDER, YLIM, NEGATIVE):
...
def __main__():
	...
	"\nthe coverage sum in a sliding window.")
	# Wrapper options. 
	parser.add_option( '-p', '--pileup',		dest='pileup',			default=None,		help='The pileup column selected file (tabulated)')
	parser.add_option( '-w', '--window',		dest='window',			default='100000',	help='The window to calculate sum')
	...
	for ch in listChr:
		...
		while debut < chr_fin:
			...
			taille = float(len(ListVal[debut:temp_fin]))
			cov = (Value/taille)*WIN
			outfile.write('\t'.join([ch, str(debut+1), str(temp_fin), str(cov)])+'\n')
			if len(list_cov) == 0:
				list_cov.append(cov)
			list_cov.append(cov)
		...
		if options.draw == 'y':
			if options.title == 'y':
				draw_plot(np.array(list_pos), np.array(list_cov), options.out+ch+'.'+options.outtype, options.out+'-'+ch, options.FillUnder, options.Ylim, options.negative)
			else:
				draw_plot(np.array(list_pos), np.array(list_cov), options.out+ch+'.'+options.outtype, ch, options.FillUnder, options.Ylim, options.negative)
		os.system("pmap %s | grep total | sed 's/ total/RAM used:/' | sed 's:  \+: :'" % V)
